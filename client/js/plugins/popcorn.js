...
Popcorn.js v0.7, modified by Brian Chirls:
- support frame animation in the core
- added `paused` to media property getters
- `previousUpdateTime` starts at -1 (Popcorn ticket #647)
- check for 'addEventListener' to prevent failure in IE8
- todo: .enable and .disable force redraw immediately, even if media has not advanced

All the above should be fixed in Popcorn.js v0.9
plan is to replace this with 0.9 when it's ready
*/
...
(function(global, document) {
  ...
  //if addEventListener doesn't work, just abandon the whole thing
      ...
      var isReady = function( that ) {
        ...
        if ( that.media.readyState >= 2 ) {
          ...
          that.media.addEventListener( "timeupdate", function( event ) {
            that.data.timeupdateEvent = event;
          }, false);
          ...
          function renderFrame() {
            var currentTime    = that.media.currentTime,
                ...
                event          = that.data.timeupdateEvent;
  ...
  Popcorn.extend(Popcorn.p, (function() {
      ...
      var methods = "load play pause currentTime playbackRate mute volume duration paused",
          ret = {};
  ...
  Popcorn.plugin = function( name, definition, manifest ) {
    ...
    var pluginFn = function( setup, options ) {
      ...
      // if a load method was declared, add this to the queue of items that need to be loaded
      //if ( "load" in setup && typeof setup.load === "function" ) {
